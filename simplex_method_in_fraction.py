...
class Arguments:
    def __init__(self):
        ...
        self.obj_atrributes = [5, 6]  # [5, 2] #an array of objective function coefficients----------5x_1 + 2x_2
        self.decision_variable_rhs = [5, 28]  # [6, 0] # right-hand side entries
        self.decision_variable_lhs = [[1, 1], [4, 7]]  # [[1, 1],[1, -1 ]] #coefficients for the left-hand-sides of the constraints
...
class MaximizationSimplex:
    ...
    def calculate_ratio_quantities_replacements(self, entering_idx):
        ...
        if fetched_details:
            ...
            self.stdform.rhs[leaving_row_indx + 1] = Fraction(
                self.stdform.rhs[leaving_row_indx + 1],  initial_pivot_row[entering_idx])
            pivot_row = [Fraction(__p, initial_pivot_row[entering_idx]) for __p in  initial_pivot_row]
